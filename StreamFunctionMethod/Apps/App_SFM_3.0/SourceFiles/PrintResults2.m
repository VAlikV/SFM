function PrintResults2(UI, R, L, px, py, axial, Fi, Ll, II, Red1, Blue1, Red2, Blue2, mode) % Инетерполяция потоковой функции и контур с током
...
switch mode
    ...
    case 3 % "Потоковая функция + Grad"
        ...
        %box(UI,"on")
        hold(UI,"on")
        quiver(UI,Fi,Ll*1000,px,py,0.2,'*r', 'MarkerSize', 2, 'LineWidth', 0.5)
        xlabel(UI,'fi'), ylabel(UI,'L, мм')
        colorbar(UI)
        hold(UI,"off")
