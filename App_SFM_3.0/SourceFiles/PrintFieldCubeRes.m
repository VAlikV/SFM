function PrintFieldCubeRes(UI, lx, ly, lz, nx, ny, nz, centerFOV, Bdes, level, plane, Lx, Ly, Lz, centerROI) % Отрисовка целевого поля
...
if (plane == "XY") % ---------------------------------------------------------- XY
    ...
    xlabel(UI,'x, мм'), ylabel(UI,'y, мм'), title(UI,strcat('Полученное поле, мТ, z = ', num2str(level*1000), 'мм')) 
elseif (plane == "XZ") % ------------------------------------------------------ XZ 
    ...
    xlabel(UI,'x, мм'), ylabel(UI,'z, мм'), title(UI,strcat('Полученное поле, мТ, y = ', num2str(level*1000), 'мм'))
elseif (plane == "YZ") % ------------------------------------------------------ YZ
    ...
    xlabel(UI,'y, мм'), ylabel(UI,'z, мм'), title(UI,strcat('Полученное поле, мТ, x = ', num2str(level*1000), 'мм'))
else
